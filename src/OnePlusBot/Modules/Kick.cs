    ...
    public class KickModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task BanAsync(IGuildUser user,[Remainder] string reason = null)
        {
            if (user.IsBot)
            {
                ...
                await Context.Channel.EmbedAsync(new EmbedBuilder().WithDescription("You humans can't make us harm each other."));
                return;
            }
            ...
            if (user.GuildPermissions.PrioritySpeaker)
            {
                ...
                await Context.Message.RemoveAllReactionsAsync();
                await Context.Message.AddReactionAsync(EmoteFalse);
                await Context.Channel.EmbedAsync(new EmbedBuilder().WithDescription("You can not kick authorities."));
                return;
            }
